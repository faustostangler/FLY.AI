...
class TestPrometheusTelemetryAdapter:
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.COMPANIES_SYNCED_TOTAL.labels"
    )
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.COMPANIES_BY_SECTOR.labels"
    )
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.COMPANIES_BY_SEGMENT.labels"
    )
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.SYNC_DURATION_SECONDS.labels"
    )
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.DATE_PARSING_FAILURES.labels"
    )
    def test_increment_date_parsing_failures(self, mock_labels, adapter):
        ...
        mock_labels.assert_called_once_with(
            field="dateListing", source="invalid format"
        )
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.NETWORK_TRANSMIT_BYTES_TOTAL.labels"
    )
    ...
    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.DATA_VALIDATION_ERRORS.labels"
    )
    def test_increment_data_validation_error(self, mock_labels, adapter):
        ...
        mock_labels.assert_called_once_with(
            entity="Company", field="cnpj", reason="invalid schema"
        )
        mock_labels.return_value.inc.assert_called_once()

    @patch(
        "shared.infrastructure.adapters.prometheus_telemetry.GENERIC_SYNC_ERRORS.labels"
    )
