...
class TestTracing:
    ...
    def test_otel_log_filter_injects_ids_when_recording(self):
        ...
        record = logging.LogRecord("test_logger", logging.INFO, "path", 10, "Test Message", (), None)
        
        with patch("shared.infrastructure.monitoring.tracing.trace.get_current_span") as mock_get_span:
            mock_span = MagicMock()
    ...
    def test_setup_tracing_happy_path(
        self, mock_resource, mock_provider, mock_exporter, mock_processor, mock_set_provider
    ):
    ...
    def test_setup_tracing_graceful_exporter_failure(
        self, mock_resource, mock_provider, mock_exporter, mock_processor, mock_set_provider, caplog
    ):
    ...
    def test_otel_log_filter_ignores_valid_trace_id_if_not_recording(self):
        ...
        with patch("shared.infrastructure.monitoring.tracing.trace.get_current_span", return_value=mock_span):
            log_filter = OTelLogFilter()
            ...
            assert record.trace_id == "00000000000000000000000000000000", "Falha Crítica: LogFilter aceitou um span que não está a gravar!"
            assert record.span_id == "0000000000000000", "Falha Crítica: LogFilter vazou o span_id!"
