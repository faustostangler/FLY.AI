...
async def test_fetch_initial_companies_http_error(mock_playwright_infrastructure):
    ...
    with pytest.raises(Exception, match="Failed to fetch initial companies page -1: 500"):
        async with data_source:
            await data_source.fetch_initial_companies()
...
@pytest.mark.asyncio
async def test_unmanaged_fallback_context_initial_and_details(mock_playwright_infrastructure):
...
@pytest.mark.asyncio
async def test_fetch_initial_companies_pagination_branch(mock_playwright_infrastructure):
    ...
    def mock_get_side_effect(endpoint, **kwargs):
        ...
        if p_num == -1:
            resp.body = AsyncMock(return_value=json.dumps(data_page_minus_one).encode("utf-8"))
        elif p_num == 0:
            resp.body = AsyncMock(return_value=json.dumps(data_page_zero).encode("utf-8"))
            
        return resp
        
    mock_playwright_infrastructure.request.get = AsyncMock(side_effect=mock_get_side_effect)
