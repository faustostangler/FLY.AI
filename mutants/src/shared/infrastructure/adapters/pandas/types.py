...
def ensure_list_col(s: pd.Series) -> pd.Series:
    args = [s]# type: ignore
    kwargs = {}# type: ignore
    return _mutmut_trampoline(x_ensure_list_col__mutmut_orig, x_ensure_list_col__mutmut_mutants, args, kwargs, None)
...
def x_ensure_list_col__mutmut_3(s: pd.Series) -> pd.Series:
    return s.apply(lambda v: v if isinstance(v, (list, tuple)) else ([] if v is not None else [v]))
