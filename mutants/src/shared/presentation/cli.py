...
def _create_sync_use_case():
    args = []# type: ignore
    kwargs = {}# type: ignore
    return _mutmut_trampoline(x__create_sync_use_case__mutmut_orig, x__create_sync_use_case__mutmut_mutants, args, kwargs, None)
...
async def _run_sync_companies():
    args = []# type: ignore
    kwargs = {}# type: ignore
    return await _mutmut_trampoline(x__run_sync_companies__mutmut_orig, x__run_sync_companies__mutmut_mutants, args, kwargs, None)
...
async def x__run_sync_companies__mutmut_7():
    ...
    try:
        ...
        # SOTA: EMPURRAR PARA O PUSHGATEWAY ANTES DO CONTÊINER MORRER
        from prometheus_client import push_to_gateway, REGISTRY
        pushgateway_url = os.getenv("XXPUSHGATEWAY_URLXX", "http://metrics-ingestion:9091")
...
async def x__run_sync_companies__mutmut_9():
    ...
    try:
        ...
        # SOTA: EMPURRAR PARA O PUSHGATEWAY ANTES DO CONTÊINER MORRER
        from prometheus_client import push_to_gateway, REGISTRY
        pushgateway_url = os.getenv("PUSHGATEWAY_URL", "XXhttp://metrics-ingestion:9091XX")
...
async def x__run_sync_companies__mutmut_16():
    ...
    try:
        ...
        try:
            logger.info(f"Empurrando métricas para o Pushgateway em {pushgateway_url}...")  # pragma: no mutate
            # 'job' é a etiqueta que agrupará essas métricas lá no Prometheus
            push_to_gateway(pushgateway_url, job='worker_sync_companies', )
            logger.info("Métricas empurradas com sucesso para o Pushgateway (metrics-ingestion).")  # pragma: no mutate
        except Exception as pg_error:
            # Não queremos que uma falha na telemetria quebre o job principal, apenas logamos
            logger.error(f"Falha ao enviar métricas para o Pushgateway (metrics-ingestion): {pg_error}")  # pragma: no mutate
...
async def x__run_sync_companies__mutmut_17():
    ...
    try:
        ...
        try:
            logger.info(f"Empurrando métricas para o Pushgateway em {pushgateway_url}...")  # pragma: no mutate
            # 'job' é a etiqueta que agrupará essas métricas lá no Prometheus
            push_to_gateway(pushgateway_url, job='XXworker_sync_companiesXX', registry=REGISTRY)
            logger.info("Métricas empurradas com sucesso para o Pushgateway (metrics-ingestion).")  # pragma: no mutate
        except Exception as pg_error:
            # Não queremos que uma falha na telemetria quebre o job principal, apenas logamos
            logger.error(f"Falha ao enviar métricas para o Pushgateway (metrics-ingestion): {pg_error}")  # pragma: no mutate
