...
def mock_playwright():
    ...
    with patch("infrastructure.adapters.scrapers.playwright_b3_scraper.async_playwright") as mock_pw:
...
async def test_fetch_initial_companies_success(mock_playwright):
    ...
    scraper = PlaywrightB3Scraper(headless=True)
    results = await scraper.fetch_initial_companies()
...
async def test_fetch_company_details_success(mock_playwright):
    ...
    scraper = PlaywrightB3Scraper(headless=True)
    details = await scraper.fetch_company_details("9512")
    
    assert details["sector"] == "Petróleo"
    assert details["codeCVM"] == "9512"

def test_create_token():
    scraper = PlaywrightB3Scraper()
    payload = {"test": "data"}
    token = scraper._create_token(payload)
