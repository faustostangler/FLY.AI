...
class TestCLICommandRouting:
    ...
    def test_create_sync_use_case(self):
        ...
        modules_patcher = patch.dict('sys.modules', {
            'shared.infrastructure.database.connection': MagicMock(engine=mock_engine, SessionLocal=mock_session_local),
            'companies.infrastructure.adapters.database.models': MagicMock(Base=mock_base),
            'companies.infrastructure.adapters.database.postgres_company_repository': MagicMock(PostgresCompanyRepository=mock_repo),
            'companies.infrastructure.adapters.data_sources.playwright_b3_data_source': MagicMock(PlaywrightB3DataSource=mock_data_source),
            'companies.application.use_cases.sync_b3_companies': MagicMock(SyncB3CompaniesUseCase=mock_use_case),
            'shared.infrastructure.adapters.prometheus_telemetry': MagicMock(PrometheusTelemetryAdapter=mock_telemetry),
        })
        ...
        with modules_patcher:
            ...
            mock_data_source.assert_called_once_with(telemetry=mock_telemetry.return_value)
    ...
    async def test_run_sync_companies_success(self, mock_getenv, mock_push, mock_create):
    ...
    @patch("shared.presentation.cli._create_sync_use_case")
    @patch("prometheus_client.push_to_gateway")
    async def test_run_sync_companies_failure_still_cleans_up(self, mock_push, mock_create):
    ...
    @patch("shared.presentation.cli._create_sync_use_case")
    @patch("prometheus_client.push_to_gateway")
    async def test_run_sync_companies_pushgateway_failure(self, mock_push, mock_create, caplog):
    ...
    @pytest.mark.skip(reason="Subprocess coverage conflict under mutmut stats collection")
    def test_run_as_main_block(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "coverage", "run", "-a", str(cli_path), "unknown-command"],
            capture_output=True,
            text=True
        )
